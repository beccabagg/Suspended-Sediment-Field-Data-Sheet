...
<div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin-bottom: 20px; border-left: 5px solid #4682B4;">
  ...
  <div style="margin-left: 15px;">
    ...
    <ol>
      ...
      <li><strong>EWI Sampling:</strong> For EWI sampling, you can enter data up to 4 sets (A, B, C, D) at the same section widths.</li>
...
server <- function(input, output, session) {
                  ...
                  tabsetPanel(id = paste0(method_id, "_tabs"),
                              ...
                              tabPanel("Set A",
                                       fluidRow(
                                         ...
                                         column(4, selectInput(paste0(method_id, "_sampler_a"), "Sampler Type:", sampler_types))
                                       ),
                              ...
                              tabPanel("Set B",
                                       fluidRow(
                                         ...
                                         column(4, selectInput(paste0(method_id, "_sampler_b"), "Sampler Type:", sampler_types))
                                       ),
                              ...
                              tabPanel("Set C",
                                       fluidRow(
                                         ...
                                         column(4, selectInput(paste0(method_id, "_sampler_c"), "Sampler Type:", sampler_types))
                                       ),
                              ...
                              tabPanel("Set D",
                                       fluidRow(
                                         column(4, numericInput(paste0(method_id, "_num_verticals_d"),
                                                                "Number of Verticals:", value = 10, min = 1, max = 40, step = 1)),
                                         column(4, textInput(paste0(method_id, "_start_time_d"), "Start Time:", value = "")),
                                         column(4, textInput(paste0(method_id, "_end_time_d"), "End Time:", value = "")),
                                         column(4, selectInput(paste0(method_id, "_sampler_d"), "Sampler Type:", sampler_types))
                                       ),
                                       textOutput(paste0(method_id, "_avg_time_d")),
                                       hr(),
                                       h4("Sampling Sections"),
                                       DTOutput(paste0(method_id, "_section_table_d"))
                              )
                  )
              )
          )
          ...
          div(class = "panel panel-default",
              ...
              div(class = "panel-body",
                  fluidRow(
                    ...
                    column(4, selectInput(paste0(method_id, "_sampler"), "Sampler Type:", sampler_types))
                  ),
  ...
  # Calculate sampling sections for methods
  observe({
    req(input$selected_methods)
    ...
    for(method in input$selected_methods) {
      ...
      if(method %in% c("EWI Iso", "EWI Non-Iso")) {
        # For EWI methods with sets A, B, C, and D
        local({
          local_method <- method
          ...
          for(set in sets) {
            num_verticals_id <- paste0(local_method_id, "_num_verticals_", set)
            ...
            section_table_id <- paste0(local_method_id, "_section_table_", set)
            ...
            observeEvent(c(input[[num_verticals_id]], input$left_edge, input$right_edge), {
              ...
              # Render the table for this specific method set
              output[[section_table_id]] <- renderDT({
                DT::datatable(
                  data,
                  options = list(
                    pageLength = 10,
